...
const katSlug = route.params.kategoria as string
const { data: lista, error } = await useFetch(`/api/kategorie/${katSlug}`)

const katNazwa = katSlug.replace(/-/g, ' ').replace(/\b\w/g, (c) => c.toUpperCase())

useHead({
  title: `Kategoria: ${katNazwa} | Katalog kościołów`,
  meta: [{ name: 'description', content: `Kościoły w kategorii ${katNazwa}.` }],
})
</script>
...
<template>
  <div class="page">
    <nav class="breadcrumb">
      ...
      <span>/</span>
      <NuxtLink to="/kategorie">Kategorie</NuxtLink>
      <span>/</span>
      <span>{{ katNazwa }}</span>
    </nav>
    <h1>Kategoria: {{ katNazwa }}</h1>
    ...
    <p v-else-if="lista && lista.length === 0" class="empty">Brak kościołów w tej kategorii.</p>
