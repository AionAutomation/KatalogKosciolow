...
const { data: lista } = await useFetch('/api/miasta')

useHead({
  title: 'Miasta | Katalog kościołów',
  meta: [{ name: 'description', content: 'Kościoły według miast. Wybierz miasto.' }],
})
</script>
...
<template>
  <div class="page">
    ...
    <ul v-if="lista?.length" class="link-list">
      <li v-for="m in lista" :key="m.id">
        <NuxtLink v-if="m.slug" :to="`/miasta/${m.slug}`">{{ m.nazwa }}</NuxtLink>
      </li>
    </ul>
    <p v-else-if="lista && lista.length === 0" class="empty">Brak miast w bazie.</p>
